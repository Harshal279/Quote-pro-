    ...
    try:
        ...
        if rows:
            try:
                ...
                # seq column may not exist in DB yet — retry without it
                rows_no_seq = [{k: v for k, v in r.items() if k != "seq"} for r in rows]
...
def load_history_file(user_key: str, filename: str) -> dict:
    ...
    # Try ordering by seq first (explicit order), fall back to created_at
    try:
        result = (
            supabase.table("chats")
